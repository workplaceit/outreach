...
function renderTitleTags() {
  ...
  for (var i = 0; i < titleTags.length; i++) {
    html += '<span class="title-pill">' + titleTags[i] + '<span class="title-pill-x" onclick="removeTitle(' + i + ')">&times;</span></span>';
  }
  wrap.innerHTML = html;
}

function removeTitle(i) {
  titleTags.splice(i, 1);
  renderTitleTags();
}
...
async function runSearch(page) {
  ...
  var perPage = parseInt(document.getElementById('f-perpage').value);
  var seniority = Array.from(document.querySelectorAll('.filter-tag.on')).map(function(t) {
    return t.textContent.toLowerCase().replace(/-/g,'_').replace('c_level','c_suite');
  });
  var body = {
    page: curPage, per_page: perPage,
  ...
  try {
    ...
    leads = data.people.map(function(p) {
      return {
        id: p.id,
    ...
    renderLeads();
    renderPagination(data.pagination, perPage);
...
function renderLeads() {
  ...
  for (var i = 0; i < leads.length; i++) {
    ...
    rows += '<td>' + (l.email ? '<span class="badge-verified" style="display:inline-flex;padding:3px 9px;border-radius:50px;font-size:11px;font-weight:700;background:#dcfce7;color:#15803d">' + l.email + '</span>' : '<span style="display:inline-flex;padding:3px 9px;border-radius:50px;font-size:11px;font-weight:700;background:#fef9c3;color:#854d0e">Not available</span>') + '</td>';
...
function renderProspects(filter) {
  ...
  if (!list.length) { g.innerHTML = '<div style="color:var(--text2);grid-column:1/-1;padding:32px;text-align:center">' + (prospects.length ? 'No match.' : 'No prospects yet.') + '</div>'; return; }
  var html = '';
  for (var i = 0; i < list.length; i++) {
    ...
    html += '<div class="prospect-body"><div class="prospect-meta"><div class="meta-chip">' + (p.size||'') + '</div><div class="meta-chip">' + (p.location||'') + '</div>' + (p.email ? '<div class="meta-chip">' + p.email + '</div>' : '') + '</div>';
    ...
    html += '<div class="prospect-actions"><button class="btn btn-primary btn-sm" onclick="goR('' + p.id + '')">Research</button><button class="btn btn-outline btn-sm" onclick="goO('' + p.id + '')">Outreach</button><button class="btn btn-ghost btn-sm" onclick="nextStage('' + p.id + '')">Next Stage</button></div>';
    html += '</div></div>';
  }
  g.innerHTML = html;
...
function renderRL() {
  ...
  for (var i = 0; i < prospects.length; i++) {
    ...
    html += '<div class="avatar" style="background:' + gc(i) + ';width:34px;height:34px;font-size:12px;flex-shrink:0">' + p.av + '</div>';
    html += '<div><div class="r-item-name">' + p.name + (p.aiS ? ' ✓' : '') + '</div><div class="r-item-co">' + p.company + '</div></div></div>';
  }
...
function runR(id) {
  ...
  setTimeout(function() {
    ...
    p.insights = [
      ...
      { t: 'Located in ' + (p.location||'your area') + ' - local opportunity' },
      { t: p.email ? 'Email: ' + p.email : 'LinkedIn outreach recommended' },
      { t: 'Custom intro message generated below' }
    ];
    p.msg = 'Hi ' + p.name.split(' ')[0] + ', I noticed ' + p.company + ' is growing and wanted to reach out. We help companies in ' + (p.location||'your area') + ' keep IT off their plate - 24/7 monitoring, no surprises, local team. Would love to show how we helped similar businesses. Open to a quick 15-min call?';
    p.stage = 'research';
...
function showRResult(p) {
  ...
  html += '<div style="position:relative"><div style="font-size:18px;font-weight:800;color:#fff">' + p.name + '</div><div style="font-size:13px;color:#a0b4cc">' + p.title + ' at ' + p.company + '</div>' + (p.email ? '<div style="font-size:12px;color:var(--cyan);margin-top:2px">' + p.email + '</div>' : '') + '</div>';
  html += '<button class="btn btn-primary btn-sm" style="margin-left:auto;position:relative" onclick="goO('' + p.id + '')">Compose Outreach</button></div>';
  html += '<div class="r-card-body"><div class="r-section"><h3>AI Insights</h3>';
  (p.insights||[]).forEach(function(ins) { html += '<div class="r-insight"><span style="margin-right:6px;color:var(--cyan)">&#10022;</span><span>' + ins.t + '</span></div>'; });
...
function renderOutreach() {
  ...
  for (var i = 0; i < prospects.length; i++) {
    ...
    html += '<div class="avatar" style="background:' + gc(i) + ';width:36px;height:36px;font-size:12px;flex-shrink:0">' + p.av + '</div>';
    html += '<div class="o-item-info"><div class="o-item-name">' + p.name + '</div><div class="o-item-prev">' + (p.msg ? p.msg.substring(0,50)+'...' : 'No message yet') + '</div></div></div>';
  }
...
function selectO(id) {
  ...
  html += '<div class="c-body">' + (mode==='email' ? '<input class="filter-input" placeholder="Subject..." style="margin-bottom:8px" value="Re: ' + p.company + '"/>' : '') + '<textarea class="c-textarea" id="msg-' + p.id + '" oninput="updCC('' + p.id + '')">' + (p.msg||'') + '</textarea></div>';
  html += '<div class="c-footer"><span class="char-count" id="cc-' + p.id + '">' + (p.msg||'').length + ' chars' + (mode==='linkedin' ? ' / 300 limit' : '') + '</span>';
  html += '<div style="display:flex;gap:8px"><button class="btn btn-ghost btn-sm" onclick="regenO('' + p.id + '')">Regenerate</button><button class="btn btn-primary btn-sm" onclick="sendO('' + p.id + '','' + mode + '')">' + (mode==='linkedin'?'Open LinkedIn':'Send Email') + '</button></div></div></div>';
