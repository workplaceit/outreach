...
function renderTitleTags(){
  var w=document.getElementById("titles-tags"),h="";
  for(var i=0;i<titleTags.length;i++){
    ...
    h+=titleTags[i];
    h+='<span onclick="removeTitle('+i+')" style="cursor:pointer;font-size:15px;line-height:1;opacity:.6;margin-left:4px">&times;</span></span>';
  }
  w.innerHTML=h;
}
...
async function runSearch(page){
  curPage=page||1;
  document.getElementById("leads-tbody").innerHTML="<tr><td colspan=7><div class=loading-bar><div class=spinner></div>Searching Apollo...</div></td></tr>";
  ...
  var sen=Array.from(document.querySelectorAll(".filter-tag.on")).map(function(t){return t.textContent.toLowerCase().replace(/-/g,"_").replace("c_level","c_suite");});
  var body={page:curPage,per_page:pp,person_seniorities:sen.length?sen:undefined,organization_locations:loc?[loc]:undefined,contact_email_status:["verified","guessed","unavailable","bounced","pending_manual_fulfillment"]};
  if(titleTags.length)body.person_titles=titleTags;
  if(ind)body.q_organization_keyword_tags=[ind];
  try{
    var res=await fetch(PROXY+"/apollo/mixed_people/search",{method:"POST",headers:{"Content-Type":"application/json","X-Apollo-Key":APOLLO_KEY},body:JSON.stringify(body)});
    var data=await res.json();
    if(data.error||!data.people){document.getElementById("leads-error").innerHTML="<div class=error-box>Apollo error: "+(data.error||data.message||"Unknown")+"</div>";document.getElementById("leads-tbody").innerHTML="";return;}
    leads.length=0;
    data.people.forEach(function(p){leads.push({id:p.id,name:(p.first_name||"")+" "+(p.last_name||""),title:p.title||"",company:p.organization?p.organization.name:"",size:p.organization?fmtSz(p.organization.estimated_num_employees):"",location:[p.city,p.state].filter(Boolean).join(", ")||p.country||"",email:p.email||"",li:p.linkedin_url||"",av:((p.first_name||"?")[0]+(p.last_name||"?")[0]).toUpperCase()});});
  ...
  }catch(e){document.getElementById("leads-error").innerHTML="<div class=error-box>Error: "+e.message+"</div>";document.getElementById("leads-tbody").innerHTML="";}
}
function fmtSz(n){if(!n)return"Unknown";if(n<11)return"1-10 emp";if(n<51)return"11-50 emp";if(n<201)return"51-200 emp";if(n<501)return"201-500 emp";return"500+ emp";}
function renderPagination(pag,pp){if(!pag)return;var pages=Math.ceil(pag.total_entries/pp);var h="Page "+curPage+" of "+pages.toLocaleString()+" &nbsp;";if(curPage>1)h+="<a onclick=\"runSearch("+(curPage-1)+")\">&larr; Prev</a> &nbsp;";if(curPage<pages)h+="<a onclick=\"runSearch("+(curPage+1)+")\"">Next &rarr;</a>";document.getElementById("pagination").innerHTML=h;}
function renderLeads(){
  var tb=document.getElementById("leads-tbody");
  if(!leads.length){tb.innerHTML="<tr><td colspan=7><div class=loading-bar>No leads found.</div></td></tr>";return;}
  var rows="";
  for(var i=0;i<leads.length;i++){
    ...
    rows+="<td><div class=lead-name-cell>";
    rows+="<div class=avatar style=\"background:"+bg+"\">"+l.av+"</div>";
    rows+="<div><div class=lead-name>"+l.name+"</div><div class=lead-sub>"+l.title+"</div></div></div></td>";
    rows+="<td>"+(l.company||"-")+"</td>";
    rows+="<td>"+(l.size||"-")+"</td>";
    rows+="<td>"+(l.location||"-")+"</td>";
    rows+=l.email
      ?"<td><span style=\"display:inline-flex;padding:3px 9px;border-radius:50px;font-size:11px;font-weight:700;background:#dcfce7;color:#15803d\">"+l.email+"</span></td>"
      :"<td><span style=\"display:inline-flex;padding:3px 9px;border-radius:50px;font-size:11px;font-weight:700;background:#fef9c3;color:#854d0e\">Not available</span></td>";
...
function addP(id){var l=leads.find(function(x){return x.id===id;});if(!l||prospects.find(function(p){return p.id===id;}))return;prospects.push(Object.assign({},l,{stage:"new"}));leads.splice(leads.indexOf(l),1);toast(l.name+" added to Prospects");updateStats();renderLeads();renderProspects();renderRL();renderOutreach();}
...
function renderProspects(filter){
  ...
  if(!list.length){g.innerHTML="<div style=\"color:var(--text2);grid-column:1/-1;padding:32px;text-align:center\">"+(prospects.length?"No match.":"No prospects yet. Add from Find Leads.")+"</div>";return;}
  var html="";
  for(var i=0;i<list.length;i++){
    var p=list[i];
    html+="<div class=prospect-card><div class=p-card-hdr>";
    html+="<div class=avatar style=\"background:"+gc(i)+";width:44px;height:44px;font-size:14px;position:relative\">"+p.av+"</div>";
...
function renderRL(){var html="";for(var i=0;i<prospects.length;i++){var p=prospects[i];html+="<div class=\"r-item "+(activeR===p.id?"active":"")+"\" onclick=\"selectR('"+p.id+"')\">";html+="<div class=avatar style=\"background:"+gc(i)+";width:34px;height:34px;font-size:12px;flex-shrink:0\">"+p.av+"</div>";html+="<div><div class=r-item-name>"+p.name+(p.aiS?" ✓":"")+"</div><div class=r-item-co>"+p.company+"</div></div></div>";}document.getElementById("r-list-items").innerHTML=html;}
function selectR(id){
  activeR=id;renderRL();
  ...
  html+="<div class=avatar style=\"background:"+gc(prospects.indexOf(p))+";width:52px;height:52px;font-size:17px;position:relative\">"+p.av+"</div>";
  html+="<div style=position:relative><div style=\"font-size:18px;font-weight:800;color:#fff\">"+p.name+"</div>";
  html+="<div style=\"font-size:13px;color:#a0b4cc\">"+p.title+" at "+p.company+"</div></div></div>";
  html+="<div class=r-card-body><div class=ai-load><div class=dot-pulse><span></span><span></span><span></span></div>Ready to research "+p.name+"</div>";
  html+="<button class=\"btn btn-primary\" style=\"margin-top:12px\" onclick=\"runR('"+p.id+"')\"">Run AI Research Now</button></div></div>";
...
function runR(id){
  ...
  setTimeout(function(){
    ...
    p.insights=[{t:p.company+" - decision maker with budget authority"},{t:"Located in "+(p.location||"your area")+" - local relationship opportunity"},{t:p.email?"Email available: "+p.email:"LinkedIn outreach recommended"},{t:"Custom intro message generated below"}];
...
function showRResult(p){
  ...
  html+="<div class=avatar style=\"background:"+gc(prospects.indexOf(p))+";width:52px;height:52px;font-size:17px;position:relative\">"+p.av+"</div>";
  html+="<div style=position:relative><div style=\"font-size:18px;font-weight:800;color:#fff\">"+p.name+"</div>";
  html+="<div style=\"font-size:13px;color:#a0b4cc\">"+p.title+" at "+p.company+"</div>";
  ...
  html+="<div class=msg-box><p>"+p.msg+"</p></div>";
  html+="<div style=\"display:flex;gap:8px;margin-top:10px\">";
  html+="<button class=\"btn btn-primary btn-sm\" onclick=\"goO('"+p.id+"')\"">Use This Message</button>";
  html+="<button class=\"btn btn-ghost btn-sm\" onclick=\"regenR('"+p.id+"')\"">Regenerate</button>";
...
function renderOutreach(){var html="";for(var i=0;i<prospects.length;i++){var p=prospects[i];html+="<div class=o-item onclick=\"selectO('"+p.id+"')\">";html+="<div class=avatar style=\"background:"+gc(i)+";width:36px;height:36px;font-size:12px;flex-shrink:0\">"+p.av+"</div>";html+="<div class=o-item-info><div class=o-item-name>"+p.name+"</div><div class=o-item-prev>"+(p.msg?p.msg.substring(0,50)+"...":"No message yet")+"</div></div></div>";}document.getElementById("o-items").innerHTML=html;document.getElementById("o-count").textContent=prospects.length;}
function selectO(id){
  var p=prospects.find(function(x){return x.id===id;});
  var mode=cModes[id]||"linkedin";
  var html="<div class=compose-card><div class=c-hdr><div class=c-hdr-title>";
  html+="<div class=avatar style=\"background:"+gc(prospects.indexOf(p))+";width:28px;height:28px;font-size:11px\">"+p.av+"</div>"+p.name+" - "+p.company+"</div>";
  html+="<div class=c-tabs>";
  html+="<div class=\"c-tab "+(mode==="linkedin"?"active":"")+"\" onclick=\"setMode('"+p.id+"','linkedin')\">LinkedIn</div>";
  html+="<div class=\"c-tab "+(mode==="email"?"active":"")+"\" onclick=\"setMode('"+p.id+"','email')\">Email</div>";
  html+="</div></div><div class=c-body>";
  if(mode==="email")html+="<input class=filter-input placeholder=Subject... style=\"margin-bottom:8px\" value=\"Re: "+p.company+"\"/>";
  html+="<textarea class=c-textarea id=\"msg-"+p.id+"\" oninput=\"updCC('"+p.id+"')\">"+(p.msg||"")+"</textarea></div>";
  html+="<div class=c-footer><span class=char-count id=\"cc-"+p.id+"\">"+(p.msg||"").length+" chars"+(mode==="linkedin"?" / 300 limit":"")+"</span>";
  html+="<div style=\"display:flex;gap:8px\">";
  html+="<button class=\"btn btn-ghost btn-sm\" onclick=\"regenO('"+p.id+"')\"">Regenerate</button>";
  html+="<button class=\"btn btn-primary btn-sm\" onclick=\"sendO('"+p.id+"','"+mode+"')\">"+(mode==="linkedin"?"Open LinkedIn":"Send Email")+"</button>";
  html+="</div></div></div>";
  ...
  html+="<div class=seq-item><div class=seq-num>3</div><div style=flex:1>Soft close<br><span style=\"font-size:11px;color:var(--text2)\">Day 10 - Office 365</span></div><span class=\"badge badge-pending\">Pending</span></div></div>";
...
function updateStats(){document.getElementById("stat-leads").textContent=leads.length+prospects.length;document.getElementById("stat-prospects").textContent=prospects.length;document.getElementById("stat-messaged").textContent=prospects.filter(function(p){return p.stage==="messaged"||p.stage==="replied"||p.stage==="call";}).length;document.getElementById("stat-replies").textContent=prospects.filter(function(p){return p.stage==="replied"||p.stage==="call";}).length;}
function toggleTag(el){el.classList.toggle("on");}
function toggleEl(el){el.classList.toggle("on");}
function showPage(name){document.querySelectorAll(".page").forEach(function(p){p.classList.remove("active");});document.querySelectorAll(".nav-tab").forEach(function(t){t.classList.remove("active");});document.getElementById("page-"+name).classList.add("active");var tabs={find:0,prospects:1,research:2,outreach:3,settings:4};document.querySelectorAll(".nav-tab")[tabs[name]].classList.add("active");if(name==="prospects")renderProspects();if(name==="research")renderRL();if(name==="outreach")renderOutreach();}
