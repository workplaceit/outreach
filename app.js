...
function renderTitleTags(){
  ...
  titleTags.forEach(function(t,i){
    ...
    h+=t;
    h+='<span onclick="removeTitle('+i+')" style="cursor:pointer;font-size:15px;line-height:1;opacity:.7;margin-left:4px">&times;</span></span>';
  });
  w.innerHTML=h;
}
...
async function runSearch(page){
  ...
  try{
    ...
    data.people.forEach(function(p){
      leads.push({id:p.id,name:(p.first_name||"")+" "+(p.last_name||""),title:p.title||"",
        ...
        email:p.email||"",li:p.linkedin_url||"",
...
function renderLeads(){
  ...
  for(var i=0;i<leads.length;i++){
    ...
    var addSvg='<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 5v14m-7-7h14" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/></svg>';
    var ignSvg='<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M18 6L6 18M6 6l12 12" stroke-linecap="round" stroke-width="2"/></svg>';
    rows+="<tr><td>"+chk+"</td>"
      ...
      +'<div class="icon-btn add" onclick="addP(''+l.id+'')" title="Add">'+addSvg+"</div>"
      +'<div class="icon-btn ign" onclick="ignOne(''+l.id+'')" title="Ignore">'+ignSvg+"</div>"
      +"</div></td></tr>";
  }
  tb.innerHTML=rows;
...
function runR(id){
  ...
  setTimeout(function(){
    p.aiS=p.company+" is growing and has IT infrastructure needs. "+p.name+" is a key decision-maker.";
    p.insights=[{t:p.company+" - decision maker with budget authority"},{t:"Located in "+(p.location||"your area")+" - local opportunity"},{t:p.email?"Email: "+p.email:"LinkedIn outreach recommended"},{t:"Custom intro message below"}];
    p.msg="Hi "+p.name.split(" ")[0]+", I noticed "+p.company+" is growing and wanted to reach out. We help companies in "+(p.location||"your area")+" keep IT completely off their plate - 24/7 monitoring, no surprises, local team. Open to a quick 15-min call?";
    p.stage="research";
...
function showRResult(p){
  var html='<div class=r-card><div class=r-card-top><div class=avatar style="background:'+gc(prospects.indexOf(p))+';width:52px;height:52px;font-size:17px;position:relative">'+p.av+'</div><div style=position:relative><div style="font-size:18px;font-weight:800;color:#fff">'+p.name+'</div><div style="font-size:13px;color:#a0b4cc">'+p.title+' at '+p.company+'</div>'+(p.email?'<div style="font-size:12px;color:#29abe2;margin-top:2px">'+p.email+'</div>':"")+'</div><button class="btn btn-primary btn-sm" style="margin-left:auto;position:relative" onclick="goO(''+p.id+'')">Compose Outreach</button></div><div class=r-card-body><div class=r-section><h3>AI Insights</h3>';
  ...
  html+='</div><div class=r-section><h3>Suggested LinkedIn Message</h3><div class=msg-box><p>'+p.msg+'</p></div><div style="display:flex;gap:8px;margin-top:10px"><button class="btn btn-primary btn-sm" onclick="goO(''+p.id+'')">Use This Message</button><button class="btn btn-ghost btn-sm" onclick="regenR(''+p.id+'')">Regenerate</button></div></div></div></div>';
...
function renderOutreach(){
  var html="";
  prospects.forEach(function(p,i){html+='<div class=o-item onclick="selectO(''+p.id+'')" ><div class=avatar style="background:'+gc(i)+';width:36px;height:36px;font-size:12px;flex-shrink:0">'+p.av+'</div><div class=o-item-info><div class=o-item-name>'+p.name+'</div><div class=o-item-prev>'+(p.msg?p.msg.substring(0,50)+"...":"No message yet")+"</div></div></div>";});
...
function selectO(id){
  ...
  var html='<div class=compose-card><div class=c-hdr><div class=c-hdr-title><div class=avatar style="background:'+gc(prospects.indexOf(p))+';width:28px;height:28px;font-size:11px">'+p.av+'</div>'+p.name+' - '+p.company+'</div><div class=c-tabs><div class="c-tab '+(mode==="linkedin"?"active":"")+'" onclick="setMode(''+p.id+'','linkedin')">LinkedIn</div><div class="c-tab '+(mode==="email"?"active":"")+'" onclick="setMode(''+p.id+'','email')">Email</div></div></div><div class=c-body>'+(mode==="email"?'<input class=filter-input placeholder="Subject..." style="margin-bottom:8px" value="Re: '+p.company+'"/>':"")+'<textarea class=c-textarea id="msg-'+p.id+'" oninput="updCC(''+p.id+'')">'+(p.msg||"")+'</textarea></div><div class=c-footer><span class=char-count id="cc-'+p.id+'">'+(p.msg||"").length+' chars'+(mode==="linkedin"?" / 300 limit":"")+'</span><div style="display:flex;gap:8px"><button class="btn btn-ghost btn-sm" onclick="regenO(''+p.id+'')">Regenerate</button><button class="btn btn-primary btn-sm" onclick="sendO(''+p.id+'',''+mode+'')">'+(mode==="linkedin"?"Open LinkedIn":"Send Email")+'</button></div></div></div><div class=seq-card><div class=c-hdr style="padding:12px 18px"><div class=c-hdr-title style="font-size:13px">Follow-up Sequence</div><button class="btn btn-outline btn-sm">+ Add Step</button></div><div class=seq-item><div class=seq-num>1</div><div style=flex:1>LinkedIn intro<br><span style="font-size:11px;color:var(--text2)">Day 1 - Manual</span></div><span class="badge '+(p.stage==="messaged"||p.stage==="replied"?"badge-verified":"badge-pending")+'">'+(p.stage==="messaged"||p.stage==="replied"?"Sent":"Pending")+'</span></div><div class=seq-item><div class=seq-num>2</div><div style=flex:1>Follow-up email<br><span style="font-size:11px;color:var(--text2)">Day 4 - Office 365</span></div><span class="badge badge-pending">Pending</span></div><div class=seq-item><div class=seq-num>3</div><div style=flex:1>Soft close<br><span style="font-size:11px;color:var(--text2)">Day 10 - Office 365</span></div><span class="badge badge-pending">Pending</span></div></div>';
